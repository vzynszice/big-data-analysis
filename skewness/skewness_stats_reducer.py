#!/usr/bin/env python3
"""
Skewness Stats - Reducer (Welford/Terriberry for M2, M3)

Bu reducer, mapper'lardan gelen kısmi istatistikleri (n, mean, M2, M3)
birleştirir ve final global N, Mean, M2, M3 değerlerini hesaplar.
"""
...
def combine_moments(N_A, mean_A, M2_A, M3_A, N_B, mean_B, M2_B, M3_B):
    """Terriberry'nin formüllerini kullanarak iki set momenti birleştirir."""
    N_total = N_A + N_B
    if N_total == 0:
...
def process_stats_line(parts, total_stats, first_group):
    """STATS_SKEW satırlarını işleyen fonksiyon"""
    if parts[0] == "STATS_SKEW" and len(parts) == 5:
        ...
        if n_k > 0:  # Boş istatistikleri işleme
            if first_group[0]:
                total_stats[0] = n_k
...
def print_statistics(total_n, total_mean, total_M2, total_M3):
    """İstatistikleri hesaplar ve yazdırır"""
    ...
    # Hesaplamalar
    if total_n > 1:  # Örneklem varyansı için en az 2 kayıt
    ...
    # Popülasyon varyansı ve std sapması
...
def reducer():
    """Ana reducer fonksiyonu"""
    # total_stats: [n, mean, M2, M3] olarak saklanır
